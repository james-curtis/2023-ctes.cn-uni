...
该项设计为婴幼童的健康提供了保障，同时感谢广大用户的使用！`,
  ...
  const newsBannerData = ref<NewsBannerData>(defaultData)
  async function refreshNewsBannerData(): Promise<typeof newsBannerData> {
    const data: NewsBannerData = await getNews()
    newsBannerData.value = data
    return newsBannerData
  }

  return {
    newsBannerData,
    refreshNewsBannerData,
  }
}
...
export const useNewsStore = defineStore(
  'news',
  () => {
    const { newsBannerData, refreshNewsBannerData } = useBanner()
